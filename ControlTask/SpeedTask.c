...
{
    VAL_LIMIT(GimbalRef.pitch_angle_dynamic_ref , -31, 5);
...
//	if(fabs(ChariotRecognition_pitch)<1.5)
	{
		GMPPositionPID.ref = ChariotRecognition_pitch+10;
		GMPPositionPID.fdb = GMPitchEncoder.ecd_angle;
	}
	
		GMPPositionPID.kp = 60+40*(1-exp(-0.1*fabs(GMPPositionPID.ref - GMPPositionPID.fdb)));
		GMPPositionPID.ki = 0.1;//0.05//0.02//0.1
		GMPPositionPID.kd = 2;//0//6
	...
	{	
		GMPPositionPID.kp = 100;//40//60//80
		GMPPositionPID.ki = 0.03;//0.03
...
//		GMYPositionPID.fdb = -(1-exp(-fabs(ChariotRecognition_yaw)))*(ChariotRecognition_yaw)*GMYawRamp.Calc(&GMYawRamp);	
  	...
  	GMYPositionPID.ref = -ChariotRecognition_yaw;//-CR_yaw_increment ;
		...
		GMYPositionPID.kp = 100+40*(1-exp(-0.3*fabs(GMYPositionPID.ref - GMYPositionPID.fdb)));//60+30//130+40*(1-exp(-0.3*fabs(GMYPositionPID.ref - GMYPositionPID.fdb)))
...
//		Set_Gimbal_Current(CAN1,2000,3000,(int16_t)(CM7SpeedPID.output));     //yaw + pitch+BoLunMotor	
		  ...
		  Set_Gimbal_Current1(CAN2,(int16_t)(CM6SpeedPID.output),(int16_t)(CM7SpeedPID.output));   //BoLunMotor + Brake
