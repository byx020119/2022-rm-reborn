...
double yaw_ecd_angle_flag =-90;             //2022加  自由模式y轴一侧限位值
double yaw_ecd_angle_flag1 =-90;          //2022加  自由模式y轴另一侧限位值
...
void YawFreeRoation(void)
{
	if(GetWorkState()==Freedom_STATE)//粗巡逻状态 自由状态
	{
		...
		if(GMPitchEncoder.ecd_angle+4<=0)//-3
		{
		pitch_rotate_flag=0;    //转向在某个位置，p轴转向标志位0？
		}
		if(pitch_rotate_flag==1)
		{
	  GimbalRef.pitch_angle_dynamic_ref=GimbalRef.pitch_angle_dynamic_ref + 0.15f;   //p轴标志位1，p轴转向角度+0.1？
		}
		if(pitch_rotate_flag==0)
		{
	  GimbalRef.pitch_angle_dynamic_ref=GimbalRef.pitch_angle_dynamic_ref - 0.15f;   //p轴标志位0，p轴转向角度-0.1？
		}
