...
void Brake_Handle()
{
	...
	if(RC_CtrlData.rc.s2!=1&&RC_CtrlData.rc.s2!=2)//测试以及停止状态下不进此函数
		{
			if((Phototube_brake==1||(last_Chassis_Temp_Speed<0&&Chassis_Temp_Speed >0)||(last_Chassis_Temp_Speed<0&&Chassis_Temp_Speed ==0))&&change_brake==0)//左侧检测换向
			{
				Brake_flag=1;
			...
			if((Phototube_brake1==1||(last_Chassis_Temp_Speed>0&&Chassis_Temp_Speed <0)||(last_Chassis_Temp_Speed>0&&Chassis_Temp_Speed ==0))&&change_brake==0)//右侧检测换向
			{
				Brake_flag=-1;
				Phototube_brake=0;
				change_brake=1;
