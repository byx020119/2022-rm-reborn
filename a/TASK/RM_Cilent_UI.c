...
Float_Data F1;
char G_aim_mark[16][3]={"011","012","013","014","015","016","017","018","019","020","021","022","023","024","025","026"};
void UI_init()
{
	//下横线
	for(int i=0;i<7;i++)
	{
	memset(&G_aim[i],0,sizeof(G_aim[i]));
	Line_Draw(&G_aim[i],G_aim_mark[i],UI_Graph_ADD,3,UI_Color_Green,1,760,540-27*i,1160,540-27*i);	
	UI_ReFresh(1,G_aim[i]);
	}
	
	//竖线
	for(int i=0;i<3;i++)
	{
	memset(&G_aim[i+7],0,sizeof(G_aim[i+7]));
	Line_Draw(&G_aim[i+7],G_aim_mark[i+7],UI_Graph_ADD,4,UI_Color_Green,1,920+40*i,0,920+40*i,1080);	
	UI_ReFresh(1,G_aim[i+7]);
	}
	
	//上横线
	for(int i=0;i<4;i++)
	{
	memset(&G_aim[i+10],0,sizeof(G_aim[i+10]));
	Line_Draw(&G_aim[i+10],G_aim_mark[i+10],UI_Graph_ADD,4,UI_Color_Green,1,760,540+27*i,1160,540+27*i);	
	UI_ReFresh(1,G_aim[i+10]);
	}
	
		/*识别圆*/
	memset(&G1,0,sizeof(G1));//自瞄圆
	Circle_Draw(&G1,"001",UI_Graph_ADD,7,UI_Color_Yellow,1,960,540,50);
	
	/*自瞄状态*/
	memset(&G2,0,sizeof(G2));//状态圆
	Circle_Draw(&G2,"002",UI_Graph_ADD,7,UI_Color_Orange,1,960,540,60);
	UI_ReFresh(2,G1,G2);
	
	/*P轴*/
	memset(&F1,0,sizeof(F1));
	Float_Draw(&F1,"101",UI_Graph_ADD,9,UI_Color_White,24,3,4,300,660,(float)GMPitchEncoder.ecd_angle);		
	UI_ReFresh(1,F1);
}
...
void UI_ReFreshAll()
{
		...
		/*识别圆*/
		if(CameraDetectTarget_Flag)
		{
			Circle_Draw(&G1,"001",UI_Graph_ADD,7,UI_Color_Yellow,1,960,540,50);
		}
		else
		{
			Circle_Draw(&G1,"001",UI_Graph_Del,7,UI_Color_Yellow,1,960,540,50);		
		}
		...
		{
			Circle_Draw(&G2,"002",UI_Graph_Del,7,UI_Color_Orange,1,960,540,60);
		}		
		UI_ReFresh(2,G1,G2);

		/*P轴*/
		Float_Draw(&F1,"101",UI_Graph_ADD,8,UI_Color_White,24,3,4,300,660,(float)GMPitchEncoder.ecd_angle);	
		UI_ReFresh(1,F1);
		Float_Draw(&F1,"101",UI_Graph_Change,8,UI_Color_White,24,3,4,300,660,(float)GMPitchEncoder.ecd_angle);	
		UI_ReFresh(1,F1);
}
/****************************************串口驱动映射************************************/
...
int UI_ReFresh(int cnt,...)
{
   ...
   switch(cnt)
   {
	 ...
	 switch(robotState.robot_id)
	 {
		 case 6:
			{
			   datahead.Sender_ID=UI_Data_RobotID_RAerial;//UI_Data_RobotID_RStandard2;
         datahead.Receiver_ID=UI_Data_CilentID_RAerial;//UI_Data_CilentID_RStandard2; 
				 break;
			}
		 case 106:
		  {
