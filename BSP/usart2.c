...
//USART6_RX接PG9--->DMA2数据流2通道5
//USART6_TX-PG14
//c板标识串口1为串口6，标识串口2为串口1
...
void USART6_IRQHandler(void)
{
	...
	if(USART_GetITStatus(USART6, USART_IT_IDLE) != RESET)       //USART的IDLE中断;IDLE是空闲中断
	{
		...
		//Target is Memory0
		if(DMA_GetCurrentMemoryTarget(DMA2_Stream2) == 0)         //DMA_GetCurrentMemoryTarget():得到当前DMA的内存地址是0还是1
		{
			DMA_Cmd(DMA2_Stream2, DISABLE);
			usart2_this_time_rx_len = USART2_DMA1_RX_BUF_LEN - DMA_GetCurrDataCounter(DMA2_Stream2);//判断0传输情况，已传输数量
			DMA2_Stream2->NDTR = (uint16_t)USART2_DMA1_RX_BUF_LEN;         //设定1传输量
			DMA2_Stream2->CR |= (uint32_t)(DMA_SxCR_CT);                  //使能1传输
			DMA_Cmd(DMA2_Stream2, ENABLE);
