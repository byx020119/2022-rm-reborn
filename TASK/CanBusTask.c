...
int16_t  pitch_ecd_bias = -2900;//600
...
//未使用
void Send_Up_to_Down_Message(CAN_TypeDef *CANx,int8_t recog_iq)
{
    CanTxMsg tx_message;    
    ...
    tx_message.Data[0] = (uint8_t)recog_iq;
    tx_message.Data[1] = 0x00;//(uint8_t)qianshao_iq;
    tx_message.Data[2] = 0x00;//(uint8_t)recog_iq;
    tx_message.Data[3] = 0x00;//(uint8_t)(danliang_iq >> 8);
    tx_message.Data[4] = 0x00;//(uint8_t)(danliang_iq );
    tx_message.Data[5] = 0x00;//(uint8_t)doget_iq;
    tx_message.Data[6] = 0x00;
    tx_message.Data[7] = 0x00;
    CAN_Transmit(CANx,&tx_message);
}
